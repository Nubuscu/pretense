...
func (rq *ReviewQuery) QueryReviews() *TopicQuery {
...
func (rq *ReviewQuery) WithReviews(opts ...func(*TopicQuery)) *ReviewQuery {
	query := &TopicQuery{config: rq.config}
...
func (rq *ReviewQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Review, error) {
	...
	if query := rq.withReviews; query != nil {
		if err := rq.loadReviews(ctx, query, nodes,
			func(n *Review) { n.Edges.Reviews = []*Topic{} },
			func(n *Review, e *Topic) { n.Edges.Reviews = append(n.Edges.Reviews, e) }); err != nil {
			return nil, err
		}
	}
...
func (rq *ReviewQuery) loadReviews(ctx context.Context, query *TopicQuery, nodes []*Review, init func(*Review), assign func(*Review, *Topic)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(review.ReviewsTable)
		s.Join(joinT).On(s.C(topic.FieldID), joinT.C(review.ReviewsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(review.ReviewsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(review.ReviewsPrimaryKey[0]))
...
func (rq *ReviewQuery) WithNamedReviews(name string, opts ...func(*TopicQuery)) *ReviewQuery {
	query := &TopicQuery{config: rq.config}
	for _, opt := range opts {
		opt(query)
	}
	if rq.withNamedReviews == nil {
